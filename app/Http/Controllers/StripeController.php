    ...
    public function webhook(Request $request) {
        ...
        switch ($event->type) {
            ...
            case 'checkout.session.completed':
                ...
                $orders = Order::query()->with(['orderItems'])->where(['stripe_session_id' =>
            $session['id']])->get();
            ...
            foreach ($orders as $order) {
                ...
                foreach ($order->orderItems as $orderItem){
                    ...
                    if ($options) {
                        sort($options);
                        $variation = $product->variations()->where('variation_type_options_ids', $options)
                        ->first();
            ...
            ->where('saved_for_later', false)->delete();
            break;
            default:
